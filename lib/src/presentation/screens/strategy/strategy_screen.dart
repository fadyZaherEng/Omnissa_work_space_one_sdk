...
import 'package:mofa/src/presentation/screens/strategy/skeleton/strategy_skeleton.dart';
import 'package:mofa/src/presentation/screens/strategy/widgets/strategy_card_widget.dart';
...
class StrategyScreen extends BaseStatefulWidget {
  const StrategyScreen({super.key});

  @override
  BaseState<StrategyScreen> baseCreateState() => _StrategyScreenState();
}

class _StrategyScreenState extends BaseState<StrategyScreen> {
  final List<Strategy> _strategies = [];
  final TextEditingController controller = TextEditingController();

  StrategyBloc get _bloc => BlocProvider.of<StrategyBloc>(context);
  int _skip = 0;
  ...
  void initState() {
    super.initState();
    _bloc.add(GetStrategyEvent(
      isMonitoringAndExecution: false,
      pageSize: _pageSize,
      skip: _skip,
      isStart: true,
    ...
    return RefreshIndicator(
      ...
      child: BlocConsumer<StrategyBloc, StrategyState>(
        listener: (context, state) {
          if (state is GetStrategySuccessState) {
            if (_skip == 0) {
              _strategies.clear();
            }
            _strategies.addAll(state.strategies);
            _isLoadingMore = false;
            _hasMore = state.strategies.length ==
                _pageSize; // أقل من pageSize يعني مفيش بيانات تانية
            hideLoading();
          } else if (state is GetStrategyErrorState) {
            _showMessageDialog(
              message: state.errorMessage,
              icon: ImagePaths.warning,
            );
            hideLoading();
          } else if (state is SearchStrategyState) {
            _strategies.clear();
            _strategies.addAll(state.stratgy);
          } else if (state is SortStrategyState) {
            _strategies.clear();
            _strategies.addAll(state.stratgy);
          }
        },
        builder: (context, state) {
          return Scaffold(
            ...
            body: state is GetStrategyLoadingState
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SearchTextFieldWidget(
                            ...
                            onChange: (value) {
                              _bloc.add(SearchStrategyEvent(search: value));
                            },
                            onClear: () {
                              controller.clear();
                              _bloc.add(SearchStrategyEvent(search: ''));
                            },
                          ...
                          _strategies.isEmpty
                              ...
                              : Container(
                                  ...
                                  child: ListView.separated(
                                    ...
                                    itemCount:
                                        _strategies.length + (_hasMore ? 1 : 0),
                                  ),
                                ),
                          const SizedBox(height: 16),
                        ],
  ...
  void _showSortBottomSheet(BuildContext context) async {
    Sort? selectedSort = await GetSortStrategyUseCase(injector())();
    showSortsBottomSheet(
      ...
      onSortSelected: (Sort value) {
        _bloc.add(GetSortedStrategyEvent(sort: value));
        if (value.id == 1) {
